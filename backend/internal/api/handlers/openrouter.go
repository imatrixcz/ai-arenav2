...
func (h *AIArenaHandler) OpenRouterSync(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	go func() {
		...
		h.DB.Database.Collection("openrouter_sync_logs").InsertOne(ctx, syncLog)
	}()
...
func (h *AIArenaHandler) GetSyncLogs(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	cursor, err := h.DB.Database.Collection("openrouter_sync_logs").Find(ctx, bson.M{}, findOptions)
	if err != nil {
...
func (h *AIArenaHandler) GetSyncStatus(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	// Get latest sync
	var latestSync models.OpenRouterSyncLog
	err := h.DB.Database.Collection("openrouter_sync_logs").
...
func (h *AIArenaHandler) performOpenRouterSync(ctx context.Context, syncLog *models.OpenRouterSyncLog) models.OpenRouterSyncLogStats {
...
func (h *AIArenaHandler) syncModel(ctx context.Context, apiModel models.OpenRouterModel, stats *models.OpenRouterSyncLogStats) error {
	...
	err := h.DB.Database.Collection("ai_models").FindOne(ctx,
	...
	if err == nil {
		...
		_, err = h.DB.Database.Collection("ai_models").UpdateByID(ctx, existingModel.ID, update)
		if err != nil {
			return err
		}
		stats.ModelsUpdated++
		...
		_, err = h.DB.Database.Collection("ai_models").InsertOne(ctx, newModel)
		if err != nil {
			return err
		}
		stats.ModelsNew++
...
func (h *AIArenaHandler) buildModelUpdate(apiModel models.OpenRouterModel, provider, slug string) bson.M {
...
func (h *AIArenaHandler) buildNewModel(apiModel models.OpenRouterModel, provider, slug string) models.AIModel {
	contextLength := int64(0)
	if apiModel.Context != nil {
		contextLength = *apiModel.Context
	}
	...
	eloRatings := models.AIModelELORatings{
		Global: 1200,
		Code:   1200,
		Image:  1200,
		Video:  1200,
